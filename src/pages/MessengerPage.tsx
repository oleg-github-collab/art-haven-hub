...
import {
  ...
  Pin, Users, Hash, ArrowLeft, Plus, Bell, BellOff, Mic,
  Image as ImageIcon, ChevronDown, Check, CheckCheck, Settings,
...
const tabs: { value: Tab; label: string; icon: React.ElementType }[] = [
  { value: "all", label: "Усі", icon: MessageCircle },
  { value: "private", label: "Особисті", icon: Lock },
  { value: "groups", label: "Групи", icon: Users },
  { value: "channels", label: "Канали", icon: Radio },
];
...
export default function MessengerPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-background">
      {/* Sidebar */}
      <aside
        className={`flex w-full flex-col border-r border-border bg-card sm:w-80 lg:w-96 ${
          activeChat ? "hidden sm:flex" : "flex"
        }`}
      >
        {/* Sidebar header */}
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h1 className="text-lg font-bold font-sans">Повідомлення</h1>
      ...
      <div className={`flex flex-1 flex-col ${!activeChat ? "hidden sm:flex" : "flex"}`}>
        {activeChat ? (
            ...
            <div className="flex items-center gap-3 border-b border-border px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <span className="text-xs text-muted-foreground">
                  {activeChat.type === "private" && (activeChat.online ? "онлайн" : "був(ла) нещодавно")}
                  {activeChat.type === "group" && `${activeChat.members} учасників`}
                  {activeChat.type === "channel" && `${activeChat.subscribers?.toLocaleString()} підписників`}
                </span>
              </div>
              <div className="flex gap-1">
          ...
          <div className="flex flex-1 flex-col items-center justify-center text-center">
            ...
            <h2 className="mb-2 text-xl font-semibold">Оберіть чат</h2>
            <p className="max-w-sm text-sm text-muted-foreground">
              Виберіть розмову зі списку або створіть нову, щоб почати спілкування
            </p>
          </div>
        )}
      </div>
    ...
    <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
      ...
      >
        {!isMe && chatType !== "private" && (
          <p className="mb-0.5 text-[11px] font-semibold text-primary">{msg.senderId}</p>
        )}
